...
var that           = {},  // Public API methods
    ...
    qemuExtKeyEventSupported, // Line added by Nutanix
    ...
    reconn_attempt      = 0,      // Retry attempt counter.
    max_reconn_attempts = 1,      // Max number of times to retry connecting.
...
Util.conf_defaults(conf, that, defaults, [
    ...
    // Begin Nutanix change
    ['skipVncHandshake',   'rw', 'bool', false, 'Skip ProtocolVersion state and jump to ServerInitialisation'],
    // End Nutanix change
...
function constructor() {
    ...
    ws.on('open', function() {
        if (rfb_state === "connect") {
            ...
            if (conf.skipVncHandshake) {
                updateState('ServerInitialisation', 'Skipping VNC handshake');
            } else {
    ...
    ws.on('close', function(e) {
        // Begin Nutanix changes
        if (typeof NxUtil !== 'undefined' && NxUtil.handleWsClose) {
          NxUtil.handleWsClose(e);
        }
        // End Nutanix changes
        ...
        if (rfb_state === 'disconnect') {
            ...
            if (reconn_attempt < max_reconn_attempts) {
                setTimeout(function() {
                    ++reconn_attempt;
                    ws.close();
                    updateState('connect', 'Trying to reconnect..');
                },
                1000);
            } else {
...
keyPress = function(keysym, scancode, down) {
    if (conf.view_only) { return; } // View only, skip keyboard events
  // Begin Nutanix changes
  if (scancode && qemuExtKeyEventSupported) {
    // Incorporate with changes for ESX non-us keyboard mapping
    if (typeof keysym.en_keysym !== 'undefined') {
      if (typeof keysym.keysym !== 'undefined') {
        keysym = keysym.keysym;
      } else {
        keysym = keysym.en_keysym;
      }
    }
    ws.send(qemuExtKeyEvent(keysym.keysym, scancode, down));
  } else {
    if(typeof keysym.en_keysym !== 'undefined') {
      keysym = keysym.en_keysym;
    }
    ws.send(keyEvent(keysym.keysym, down));
  }
  // End Nutanix changes
};
...
// Begin Nutanix changes
encHandlers.QEMUExtendedKeyEvent = function () {
  FBU.rects--;
  qemuExtKeyEventSupported = true;
};
// End Nutanix changes
...
qemuExtKeyEvent = function(keysym, scancode, down) {
    var arr;
    //console.log(">> qemuExtKeyEvent, keysym: " + keysym + ", scancode: " + scancode + ", down: " + down);
    arr = [255, 0]; // msg-type
    arr.push16(down);
    arr.push32(keysym);
    arr.push32(scancode);
    //console.log("<< qemuExtKeyEvent");
    return arr;
}
